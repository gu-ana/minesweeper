...
class GameGrid:
    ...
    def neighbors(self, i, j):
       ...
       """
        def coords(x, y): return [
            (x2, y2) for x2 in range(i - 1, i + 2)
            for y2 in range(j - 1, j + 2)
            if (0 <= x <= self.nrows and 0 <= y <= self.ncols and
                (x != x2 or y != y2) and (0 <= x2 < self.nrows) and
                (0 <= y2 < self.ncols))
        ]
        return coords(i, j)
    ...
    def flags_nearby(self, i, j):
        ...
        for x, y in self.neighbors(i, j):
            total_flagged += 1 if self.flagged(x, y) else 0
    ...
    def place_mine(self, i, j):
       ...
       """
        curr_square = self.square_at(i, j)
        if curr_square.mined == True:
            return
        else:
            curr_square.mined = True
        for x, y in self.neighbors(i, j):
            neighbor_square = self.square_at(x, y)
            neighbor_square.mines_nearby += 1
        self.nmines += 1
    ...
    def reveal(self, i, j):
        ...
        """
        if self.flagged(i, j) or self.revealed(i, j):
            return
        self.squares_revealed += 1
        # its important to put revealed = True before the if statement such that
        # the recursion stops early when the if condition checks for revealed(i, j)
        self.square_at(i, j).revealed = True
        if self.mined(i, j) == False and self.mines_nearby(i, j) == 0:
            for x, y in self.neighbors(i, j):
                self.reveal(x, y)
    ...
    def chording(self, i, j):
        ...
        """
        # TODO: Maybe only chord on revealed (i, j)?
        if self.flags_nearby(i, j) == self.mines_nearby(i, j):
            # cant reveal on (i, j) since it's revealed, which would immediately return
            for x, y in self.neighbors(i, j):
                self.reveal(x, y)
    ...
    def random_game(self, n_mines):
        ...
        self.reset()
        # this is probably bad practice as every time we call random game, we recalculate
        # the entire indices (population), possibly store the indices if I were to implement
        # not customizable grids but as of now left alone.
        mines = random.sample(
            list(product(range(self.nrows), range(self.ncols))), n_mines)
        for i, j in mines:
            self.place_mine(i, j)
